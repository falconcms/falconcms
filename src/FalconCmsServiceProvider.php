...
use FalconCms\Core\Console\Commands\MakeDashboardPage;
use FalconCms\Core\Console\Commands\MakePlugin;
use FalconCms\Core\Console\Commands\MakeTheme;
...
use FalconCms\Core\Console\Commands\SeedFalconCms;
use FalconCms\Core\Console\Commands\UninstallFalconCms;
use FalconCms\Core\Console\Commands\UninstallFalconDatabase;
...
class FalconCmsServiceProvider extends ServiceProvider
{
    public function boot(): void
    {
        ...
        try {
            $this->app->make(PluginManager::class)->loadActive($this->app);
        } catch (\Throwable $e) {
        ...
        $this->commands([
            FalconList::class,
            MakeDashboardPage::class,
            InstallFalconCms::class,
            UninstallFalconCms::class,
            UninstallFalconDatabase::class,
            SeedFalconCms::class,
        ...
        if (!$this->app->runningInConsole()) {
            $this->app->terminating(function () {
                ...
                try {
                    if (Cache::add('falcon_analytics_prune_lock', 1, now()->addDay())) {
    ...
    public function register(): void
    {
        ...
        $this->app->singleton(AdminMenu::class);
        ...
        $this->app->singleton(SettingsExtension::class);
        ...
        try {
            ...
            if (!$this->app->runningInConsole()) {
                $setting = DB::table('cms_settings')->where('key', 'active_theme')->first();
                if ($setting) {
    ...
    {
        try {
            return $this->app->make(PluginManager::class)->loaded();
        } catch (\Throwable $e) {
            return [];
        }
    }
