...
class BuilderShortcodeConverter
{
    ...
    private static function perTypeNames(string $kind, array $node): array
    {
        ...
        try {
            if ($kind === 'container') {
                $sc = self::containerToShortcode(self::stripChildren($node));
            } elseif ($kind === 'column') {
                $sc = self::columnToShortcode(self::stripChildren($node));
            } else {
                $sc = self::elementToShortcode(self::stripChildren($node));
            }
        } finally {
