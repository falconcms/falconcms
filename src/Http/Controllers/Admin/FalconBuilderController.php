...
class FalconBuilderController extends Controller
{
    ...
    private function assign(string $slot, ?int $sectionId, string $layout = 'global'): void
    {
        if ($layout === 'global') {
            ...
            if ($sectionId) {
                $a[$slot] = ['id' => $sectionId, 'active' => true];
            } else {
                unset($a[$slot]);
            }
        ...
        foreach ($layouts as &$l) {
            if (($l['id'] ?? null) === $layout) {
                if (!isset($l['assignments']) || !is_array($l['assignments'])) {
                    $l['assignments'] = [];
                }
                if ($sectionId) {
                    $l['assignments'][$slot] = ['id' => $sectionId, 'active' => true];
                } else {
                    unset($l['assignments'][$slot]);
                }
            }
        }
        unset($l);
    ...
    private function activePostTypes()
    {
        return PostType::query()
            ->when(Schema::hasColumn('post_types', 'is_active'), fn ($q) => $q->where('is_active', true))
            ->orderBy('id')->get();
    }
    ...
    private function taxonomyCatalogue(): array
    {
        ...
        if (Schema::hasTable('custom_taxonomies')) {
            foreach (CustomTaxonomy::query()
                ->when(Schema::hasColumn('custom_taxonomies', 'is_active'), fn ($q) => $q->where('is_active', true))
                ->orderBy('name')->get() as $t) {
                if (in_array($t->slug, ['product_category', 'product_tag', 'category', 'post_tag'], true)) {
                    continue;
                }
    ...
    private function conditionTabs(): array
    {
        $tabs = [];
        $byName = [];
        foreach ($this->activePostTypes() as $pt) {
            $byName[$pt->slug] = $pt;
        }
        ...
        foreach ($byName as $slug => $pt) {
            if (in_array($slug, ['page', 'post'], true)) {
                continue;
            }
    ...
    private function targetLabel(string $target): string
    {
        ...
        if ($target === 'author_archive') {
            return 'All Author Pages';
        }
        if (str_starts_with($target, 'all:')) {
            return 'All '.($ptNames[substr($target, 4)] ?? ucfirst(substr($target, 4)));
        }
        if (str_starts_with($target, 'archive:')) {
            return ($ptNames[substr($target, 8)] ?? ucfirst(substr($target, 8))).' Archive';
        }
        if (str_starts_with($target, 'post:')) {
            return optional(Post::find((int) substr($target, 5)))->title ?: ('Item #'.substr($target, 5));
        }
        if (str_starts_with($target, 'author:')) {
            return $this->authorName((int) substr($target, 7));
        }
        if (str_starts_with($target, 'tax:')) {
            return 'All '.($taxNames[substr($target, 4)] ?? ucfirst(substr($target, 4)));
        }
    ...
    private function termName(string $taxonomy, int $id): string
    {
        try {
            return match ($taxonomy) {
                'category' => optional(Category::find($id))->name,
                'post_tag' => optional(Tag::find($id))->name,
                'product_category' => optional(ProductCategory::find($id))->name,
                'product_tag' => optional(ProductTag::find($id))->name,
                default => optional(TaxonomyTerm::find($id))->name,
            } ?: ('Term #'.$id);
        } catch (\Throwable $e) {
            return 'Term #'.$id;
        }
    ...
    public function renameLayout(Request $request)
    {
        ...
        unset($l);
        if (!$found) {
            return response()->json(['ok' => false, 'message' => 'Layout not found.'], 404);
        }
        $this->saveCustomLayouts($layouts);
    ...
    private function taxonomyTermQuery(string $taxonomy, string $s)
    {
        $q = match ($taxonomy) {
            ...
            'product_tag' => ProductTag::query(),
            default => TaxonomyTerm::where('taxonomy_slug', $taxonomy),
        };
        if ($s !== '') {
            $q->where('name', 'like', '%'.$s.'%');
        }
