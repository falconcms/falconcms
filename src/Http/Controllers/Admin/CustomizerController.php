...
class CustomizerController extends Controller
{
        ...
        return [
            ...
            'menu' => [
                ...
                'fields' => [
                    ...
                    // header replaces that header entirely, and the Customizer says so).
                    //
                    // The navigation FONT is not among them: Typography → Navigation owns it
                    // whole — family, size, weight, line height, spacing and case — and is the
                    // single control the theme renders from. This section briefly carried its own
                    // Font Size and Font Weight; because they had to out-specify the typography
                    // rule to have any effect at all, they also silenced it, and changing the
                    // navigation size under Typography stopped doing anything. One property, one
                    // control.
                    'hr_menu_font' => [
                        'type' => 'info',
            ...
            'performance' => [
                ...
                'fields' => [
                    'to_top_enabled' => [
                        ...
                        'desc' => 'Shows a button in the corner once the reader has scrolled down, which takes them back to the top of the page.',
                        'default' => '1',
                    ],
                    'to_top_offset' => [
                        'type' => 'range',
                        'label' => 'Appear After',
                        'desc' => 'How far down the page the reader has to be before the button appears, in pixels. A short page never scrolls that far, so the button stays out of the way.',
                        'default' => '300',
                        'min' => 50,
                        'max' => 2000,
                        'step' => 50,
                        'depends' => 'to_top_enabled',
                    ],
                    'to_top_position' => [
                        'type' => 'select',
                        'label' => 'Position',
                        'desc' => 'Which corner the button sits in.',
                        'default' => 'right',
                    ...
                    'to_top_side_gap' => [
                        ...
                        'desc' => 'Space between the button and the side and bottom of the screen, in pixels.',
                        'default' => '24',
                        'min' => 0,
                        'max' => 120,
                        'depends' => 'to_top_enabled',
                    ...
                    'to_top_size' => [
                        'type' => 'range',
                        'label' => 'Size',
                        'desc' => 'Width and height of the button, in pixels.',
                        'default' => '44',
                        'min' => 28,
                        'max' => 80,
                        'depends' => 'to_top_enabled',
                    ...
                    'to_top_radius' => [
                        'type' => 'range',
                        'label' => 'Corner Radius',
                        'desc' => 'Half the size makes a circle; zero makes a square.',
                        'default' => '50',
                        'min' => 0,
                        'max' => 50,
                        'depends' => 'to_top_enabled',
                    ...
                    'to_top_icon' => [
                        'type' => 'select',
                        'label' => 'Icon',
                        'desc' => 'What the button shows.',
                        'default' => 'arrow',
                        'options' => ['arrow' => 'Arrow', 'chevron' => 'Chevron', 'caret' => 'Triangle'],
                        'depends' => 'to_top_enabled',
                    ],
                    'to_top_bg_color' => [
                        'type' => 'color',
                        'label' => 'Background',
                        'desc' => 'Leave empty to use the theme\'s primary colour.',
                        'default' => '',
                        'depends' => 'to_top_enabled',
                    ],
                    'to_top_icon_color' => [
                    ...
                    'to_top_hover_bg_color' => [
                        ...
                        'desc' => 'Leave empty and the button simply darkens a little.',
                        'default' => '',
                        'depends' => 'to_top_enabled',
                    ],
                    'to_top_show_progress' => [
                        'type' => 'toggle',
                        'label' => 'Show Reading Progress',
                        'desc' => 'Draws a ring around the button showing how far down the page the reader is.',
                        'default' => '0',
                        'depends' => 'to_top_enabled',
                    ],
                    'to_top_hide_mobile' => [
                        ...
                        'desc' => 'A phone screen has little room to spare, and a floating button can sit on top of what the reader is trying to tap.',
                        'default' => '0',
                        'depends' => 'to_top_enabled',
                    ],
                    'maintenance_mode' => [
    ...
    public function save(Request $request)
    {
        ...
        foreach ($this->sections() as $sectionKey => $sec) {
            if (in_array($sectionKey, $lockedSections)) {
                continue;
            }
            foreach ($sec['fields'] as $key => $field) {
                if (in_array($field['type'] ?? '', $nonDataTypes)) {
                    continue;
                }
    ...
    protected function optimizeImages()
    {
        ...
        foreach ($mediaItems as $media) {
            ...
            if ($autoWebp && $media->mime_type === 'image/webp') {
                continue;
            }
