 ...
 * menu itself never read it: falcon_render_special_menu_item() always drew one of three fixed
...
class SpecialMenuItemIconTest extends TestCase
{
    ...
    {
        $html = falcon_render_special_menu_item($this->specialItem('special_cart', 'fa fa-shopping-basket'));
    ...
    {
        $html = falcon_render_special_menu_item($this->specialItem('special_search', 'fa fa-magnifying-glass-plus'));
    ...
    {
        $html = falcon_render_special_menu_item($this->specialItem('special_wishlist', 'fa fa-heart-circle-plus'));
    ...
    public function test_the_default_svg_is_kept_when_no_icon_is_chosen(): void
    {
        foreach (['special_cart', 'special_search', 'special_wishlist'] as $type) {
            $html = falcon_render_special_menu_item($this->specialItem($type, null));
    ...
    {
        $html = falcon_render_special_menu_item($this->specialItem('special_cart', '"><script>alert(1)</script>'));
    ...
    {
        $html = falcon_render_special_menu_item($this->specialItem('special_cart', null));
