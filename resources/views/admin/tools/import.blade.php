    ...
    <div class="px-2">
        ...
        @if(session('import_summary'))
            ...
            <div class="bg-white border border-[#c3c4c7] shadow-sm mb-6 overflow-hidden">
                ...
                <div class="p-5">
                    <div class="grid grid-cols-3 sm:grid-cols-8 gap-3 text-center mb-4">
                        @foreach(['posts'=>'Posts','pages'=>'Pages','cpt'=>'Custom','categories'=>'Categories','tags'=>'Tags','menus'=>'Menus','layouts'=>'Layouts','skipped'=>'Skipped'] as $k => $label)
                    ...
                    @php $__new = ($s['posts']??0)+($s['pages']??0)+($s['cpt']??0)+($s['categories']??0)+($s['tags']??0)+($s['menus']??0)+($s['layouts']??0); @endphp
                    @if($__new === 0)
