        ...
        setup() {
            ...
            const dynSrcDefs = {
                text: [
                    ...
                    // Taxonomy: pick a post type, then one of ITS taxonomies. The terms render only
                    // when the post being rendered is of that type — otherwise the fallback shows.
                    { key: 'post_taxonomy',      label: 'Taxonomy',        icon: 'fa-folder-open',   group: 'Post',   subFields: [
                        { key: 'dynamic_tax_post_type', label: 'Post Type', type: 'select', optionsFrom: 'postTypes', required: true },
                        { key: 'dynamic_tax_slug',      label: 'Taxonomy',  type: 'select', optionsFrom: 'taxonomies', dependsOn: 'dynamic_tax_post_type', required: true },
                        { key: 'dynamic_tax_separator', label: 'Separator', type: 'text',   placeholder: ', ' },
                        { key: 'dynamic_tax_limit',     label: 'Max Terms', type: 'number', placeholder: 'All' },
                        { key: 'dynamic_before',        label: 'Before',    type: 'text',   placeholder: 'Text before value' },
                        { key: 'dynamic_after',         label: 'After',     type: 'text',   placeholder: 'Text after value' },
                        { key: 'dynamic_fallback',      label: 'Fallback',  type: 'text',   placeholder: 'Uncategorized' },
                    ]},
                ...
                link: [
                    ...
                    // Links to the archive of the post's own term in the chosen taxonomy — the link
                    // twin of the Taxonomy text source, so a term can be shown AND linked dynamically.
                    { key: 'taxonomy_url', label: 'Taxonomy URL', icon: 'fa-folder-open', group: 'Post', subFields: [
                        { key: 'dynamic_link_tax_post_type', label: 'Post Type', type: 'select', optionsFrom: 'postTypes', required: true },
                        { key: 'dynamic_link_tax_slug',      label: 'Taxonomy',  type: 'select', optionsFrom: 'taxonomies', dependsOn: 'dynamic_link_tax_post_type', required: true },
                        { key: 'dynamic_link_tax_which',     label: 'Which Term', type: 'select', options: [
                            { value: 'first', label: 'First term' },
                            { value: 'last',  label: 'Last term' },
                        ]},
                        { key: 'dynamic_link_tax_fallback',  label: 'Fallback URL', type: 'text', placeholder: 'e.g. /blog' },
                    ]},
            ...
            const dynSrcPreview = (s) => {
                ...
                // Taxonomy: preview real term names from the chosen taxonomy so the canvas shows
                // what the front-end will render, not a placeholder word.
                if (s.dynamic_source === 'post_taxonomy') {
                    const terms = (window.falconTaxonomyTerms || {})[s.dynamic_tax_slug] || [];
                    const limit = Math.max(0, parseInt(s.dynamic_tax_limit, 10) || 0);
                    const names = (limit ? terms.slice(0, limit) : terms.slice(0, 3)).map(t => t.name);
                    const body  = names.length ? names.join(s.dynamic_tax_separator || ', ')
                                               : (s.dynamic_fallback || 'Taxonomy');
                    return (s.dynamic_before || '') + body + (s.dynamic_after || '');
                }
            ...
            // Options for a sub-field select. Static `options` win; `optionsFrom` is resolved live
            // so the Taxonomy pickers always reflect the site's real post types and taxonomies,
            // and the taxonomy list narrows to the ones registered for the chosen post type.
            const dynSrcFieldOptions = (field) => {
                ...
                if (field.optionsFrom === 'taxonomies') {
                    const all = falconTaxonomies.value || [];
                    const pt = field.dependsOn ? (dynSrcMenu.settings?.[field.dependsOn] || '') : '';
                    // undefined = post type we have no mapping for → offer everything rather than nothing
                    const allowed = pt ? falconCptTaxonomies.value[pt] : undefined;
                    const list = (allowed === undefined) ? all : all.filter(t => allowed.includes(t.slug));
                    return list.map(t => ({ value: t.slug, label: t.name }));
                }
                return [];
            };
            // Post type changed → the previously chosen taxonomy may not belong to it any more.
            const onDynSrcFieldChange = (field) => {
                if (!dynSrcMenu.settings || !field) return;
                const def = getDynSrcDef(dynSrcMenu.configKey);
                (def.subFields || []).forEach(f => {
                    if (f.dependsOn !== field.key) return;
                    const opts = dynSrcFieldOptions(f);
                    const cur  = dynSrcMenu.settings[f.key];
                    if (!opts.some(o => o.value === cur)) {
                        dynSrcMenu.settings[f.key] = opts.length ? opts[0].value : '';
                    }
                });
            };
            const _applySubFieldDefaults = (key) => {
                ...
                (def.subFields || []).forEach(field => {
                    if (field.type !== 'select' || dynSrcMenu.settings[field.key]) return;
                    const opts = dynSrcFieldOptions(field);
                    if (opts.length) dynSrcMenu.settings[field.key] = opts[0].value;
                });
            };
            ...
            const activeContentBoxItem = ref(null);
            const FA_GROUPS = {
            ...
            // Font Awesome keeps every previous name working as an alias of the current one, so a
            // search has to match those too — otherwise an icon you know by its old name ("ambulance",
            // "trash-alt") looks missing even though it is in the list under its current name.
            const FA_ALIASES = @json(falcon_fontawesome_aliases());
            // Extra icon libraries. Nothing about them ships with the page: the icon list arrives
            // from the server and the stylesheet is injected the first time a tab is opened, so
            // registering more sets never makes the builder heavier to load.
            const ICON_SETS   = @json(falcon_icon_sets());
            const ICON_SET_URL = @json(route('admin.falcon-builder.icons', ['set' => '__SET__']));
            const iconTabs    = ['Solid', 'Regular', 'Brands', ...Object.values(ICON_SETS).map(s => s.label)];
            const iconSetIcons  = reactive({});     // label → class strings
            const iconSetLoading = reactive({});
            const iconSetBySlug = Object.entries(ICON_SETS).reduce((a, [slug, def]) => { a[def.label] = slug; return a; }, {});

            const ensureIconSet = (label) => {
                const slug = iconSetBySlug[label];
                if (!slug || iconSetIcons[label] || iconSetLoading[label]) return;
                iconSetLoading[label] = true;
                // The set's own stylesheet, once — without it the picker would show empty boxes.
                const linkId = 'falcon-iconset-' + slug;
                if (!document.getElementById(linkId)) {
                    const l = document.createElement('link');
                    l.id = linkId; l.rel = 'stylesheet';
                    l.href = '{{ asset('vendor/falcon-cms') }}/' + ICON_SETS[slug].asset;
                    document.head.appendChild(l);
                }
                fetch(ICON_SET_URL.replace('__SET__', slug), { headers: { 'Accept': 'application/json' } })
                    .then(r => r.json())
                    .then(d => { iconSetIcons[label] = d.icons || []; })
                    .catch(() => { iconSetIcons[label] = []; })
                    .finally(() => { iconSetLoading[label] = false; });
            };
            const selectIconTab = (label) => { activeIconTab.value = label; ensureIconSet(label); };

            const filteredIcons = computed(() => {
                const icons = FA_GROUPS[activeIconTab.value] || iconSetIcons[activeIconTab.value] || [];
                const q = searchIconQuery.value.trim().toLowerCase();
                const list = q ? icons.filter(icon => {
                    if (icon.toLowerCase().includes(q)) return true;
                    const name = icon.slice(icon.indexOf('fa-') + 3);
                    const alt  = FA_ALIASES[name];
                    return alt ? alt.indexOf(q) !== -1 : false;
                }) : icons;
            ...
            const iconTabCount = computed(() => (FA_GROUPS[activeIconTab.value] || iconSetIcons[activeIconTab.value] || []).length);
            ...
            const setEditingContext = (type, ci = null, coli = null, eli = null, ncoli = null, neli = null) => {
                ...
                // Open every entity on its first tab. The panel used to carry the previous tab
                // over, so after working in an element's Design tab the NEXT element you opened
                // (a duplicate, most visibly) also opened on Design — never where you'd expect to
                // start. The tab is only kept when re-targeting the entity already being edited,
                // which is what drag/resize handlers do mid-interaction.
                const cur = editingContext.value;
                const sameTarget = cur.type === type && cur.ci === ci && cur.coli === coli
                    && cur.eli === eli && cur.ncoli === ncoli && cur.neli === neli;

                editingContext.value = {
                    type, ci, coli, eli, ncoli, neli,
                    tab: sameTarget ? (cur.tab || 'content') : 'content'
                };
                if (!sameTarget) {
                    activePanelTab.value    = 'general';   // containers
                    activeColPanelTab.value = 'general';   // columns / nested columns
                    // The icon picker's search box is shared by every picker on the page, so a
                    // query typed for one element used to filter the grid of the next one you
                    // opened — it looked like that element's icons, but it was the old search.
            ...
            // Tab resetting lives in setEditingContext (it knows the exact target, indices and
            // all); this watcher only keeps the rich editors alive.
            ...
            // These mirror frontend/builder/elements/content-box.blade.php value for value.
            // Keeping the maths in one place (rather than inline in the template) is what makes
            // the canvas and the front-end stay in step as designs get added.
            const cbNum = (v, dflt) => (v === undefined || v === null || v === '' ? dflt : Number(v));
            const contentBoxCols = (el) => {
                const s = el.settings || {}, items = s.items || [];
                if (s.boxLayout === 'timeline-vertical') return 1;
                if (s.boxLayout === 'timeline-horizontal') return cbNum(s.columns, 0) > 1 ? cbNum(s.columns, 0) : Math.max(1, items.length);
                return Math.max(1, cbNum(s.columns, 1));
            };
            // Does any box carry Read More text? The Read More design controls are gated on this
            // rather than on Link Area — with the whole box linked the text still renders (as a
            // span, not a nested anchor), so its typography is just as relevant.
            const contentBoxHasMore = (el) => ((el && el.settings && el.settings.items) || [])
                .some(i => String((i && i.linkText) || '').trim() !== '');
            // Dot box: uniform for the whole element so the rail lines up, and big enough to hold
            // a glyph (which is as tall as its font-size) without bursting the circle.
            const contentBoxDot = (el) => {
                const s = el.settings || {};
                const hasIcon = (s.items || []).some(i => (i && (i.icon || i.image)));
                if (!hasIcon) return cbNum(s.timelineDotSize, 16);
                return Math.max(cbNum(s.timelineDotSize, 16), cbNum(s.iconSize, 32) + cbNum(s.iconPadding, 0) * 2 + 16);
            };
            const contentBoxDotIcon = (el, item) => {
                const s = el.settings || {};
                const size = Math.max(8, Math.min(cbNum(s.iconSize, 32), contentBoxDot(el) - 12));
                // An image needs box dimensions, a glyph needs a font-size — sizing one like the
                // other is what made the dots look deformed.
                if (item && item.image) {
                    return { width: size + 'px', height: size + 'px', objectFit: 'contain', display: 'block' };
                }
                return {
                    fontSize: size + 'px',
                    // The dot is filled with the icon colour by default, so the glyph on top of it
                    // needs its own contrast colour or it vanishes into the dot.
                    color: (item && item.iconColor) || s.timelineIconColor || '#ffffff',
                    lineHeight: 1,
                };
            };
            const contentBoxFlexAlign = (el) => {
                const a = (el.settings || {}).alignment || 'left';
                return a === 'center' ? 'center' : (a === 'right' ? 'flex-end' : 'flex-start');
            };
            const contentBoxIconStyle = (el, item) => {
                const s = el.settings || {};
                return { fontSize: cbNum(s.iconSize, 32) + 'px', color: (item && item.iconColor) || s.iconColor || '#2271b1', lineHeight: 1 };
            };
            const contentBoxIconWrapStyle = (el) => {
                const s = el.settings || {};
                const st = { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', boxSizing: 'content-box', flexShrink: 0 };
                if (s.iconBgColor) {
                    const box = cbNum(s.iconSize, 32) * 2;
                    st.width = box + 'px'; st.height = box + 'px';
                    st.backgroundColor = s.iconBgColor;
                    st.borderRadius = cbNum(s.iconBorderRadius, 50) + 'px';
                    st.padding = cbNum(s.iconPadding, 0) + 'px';
                }
                return st;
            };
            const contentBoxDotStyle = (el) => {
                const s = el.settings || {}, d = contentBoxDot(el);
                const st = {
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    width: d + 'px', height: d + 'px', borderRadius: '50%',
                    background: s.timelineDotColor || s.iconColor || '#2271b1', boxSizing: 'border-box',
                };
                if (cbNum(s.timelineDotBorderWidth, 0) > 0) {
                    st.border = cbNum(s.timelineDotBorderWidth, 0) + 'px solid ' + (s.timelineDotBorderColor || '#ffffff');
                }
                return st;
            };
            const contentBoxTitleStyle = (el) => {
                const s = el.settings || {};
                return {
                    fontFamily: s.titleFontFamily || 'inherit',
                    fontSize: getUnitVal(s.titleFontSize || 20, s.titleFontSizeUnit || 'px'),
                    fontWeight: s.titleFontWeight || '600',
                    color: s.titleColor || '#222222',
                    lineHeight: s.titleLineHeight || 1.3,
                    letterSpacing: s.titleLetterSpacing || '0px',
                    textTransform: s.titleTextTransform || 'none',
                    margin: '0 0 ' + cbNum(s.titleSpacing, 10) + 'px 0',
                };
            };
            const contentBoxContentStyle = (el) => {
                const s = el.settings || {};
                return {
                    fontFamily: s.contentFontFamily || 'inherit',
                    fontSize: getUnitVal(s.contentFontSize || 15, s.contentFontSizeUnit || 'px'),
                    fontWeight: s.contentFontWeight || '400',
                    color: s.contentColor || '#666666',
                    lineHeight: s.contentLineHeight || 1.6,
                    letterSpacing: s.contentLetterSpacing || '0px',
                    margin: 0,
                };
            };
            const contentBoxMoreStyle = (el) => {
                const s = el.settings || {};
                const st = {
                    fontFamily: s.linkFontFamily || 'inherit',
                    fontSize: getUnitVal(s.linkFontSize || 14, s.linkFontSizeUnit || 'px'),
                    fontWeight: s.linkFontWeight || '600',
                    textTransform: s.linkTextTransform || 'none',
                    textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '6px',
                    marginTop: cbNum(s.linkSpacing, 14) + 'px',
                };
                if (s.linkType === 'button') {
                    st.color = s.linkButtonTextColor || '#ffffff';
                    st.background = s.linkColor || s.iconColor || '#2271b1';
                    st.padding = cbNum(s.linkButtonPaddingY, 10) + 'px ' + cbNum(s.linkButtonPaddingX, 20) + 'px';
                    st.borderRadius = cbNum(s.linkButtonRadius, 4) + 'px';
                } else {
                    st.color = s.linkColor || s.iconColor || '#2271b1';
                }
                return st;
            };
            // Chrome (a background or a border) implies room inside the box unless a padding was
            // set explicitly — content flush against its own border reads as broken.
            const contentBoxPad = (el, item) => {
                const s = el.settings || {};
                if (s.boxPadding !== undefined && s.boxPadding !== null && s.boxPadding !== '') return cbNum(s.boxPadding, 0);
                const bg = (item && item.bgColor) || s.boxBgColor
                    || (s.items || []).some(i => i && i.bgColor);
                const hasChrome = !!bg || cbNum(s.boxBorderWidth, 0) > 0;
                return (s.boxLayout === 'classic-boxed' || hasChrome) ? 30 : 0;
            };
            // Everything between the item's outer edge and its content — what the timeline rail
            // has to reach back across when the box is padded or bordered.
            const contentBoxInset = (el) => contentBoxPad(el, null) + cbNum((el.settings || {}).boxBorderWidth, 0);
            const contentBoxItemStyle = (el, item) => {
                const s = el.settings || {};
                const st = { boxSizing: 'border-box' };
                const bg = (item && item.bgColor) || s.boxBgColor;
                if (bg) st.backgroundColor = bg;
                const pad = contentBoxPad(el, item);
                if (pad > 0) st.padding = pad + 'px';
                if (cbNum(s.boxBorderRadius, 0) > 0) st.borderRadius = cbNum(s.boxBorderRadius, 0) + 'px';
                if (cbNum(s.boxBorderWidth, 0) > 0) st.border = cbNum(s.boxBorderWidth, 0) + 'px solid ' + (s.boxBorderColor || '#e5e7eb');
                const shadows = { small: '0 1px 4px rgba(0,0,0,.08)', medium: '0 4px 14px rgba(0,0,0,.10)', large: '0 12px 30px rgba(0,0,0,.14)' };
                if (shadows[s.boxShadow]) st.boxShadow = shadows[s.boxShadow];
                return st;
            };
            const contentBoxItemLayoutStyle = (el) => {
                const s = el.settings || {}, lay = s.boxLayout || 'classic-title';
                if (lay === 'timeline-vertical') {
                    return { position: 'relative', display: 'flex', alignItems: 'flex-start', gap: cbNum(s.timelineGap, 22) + 'px' };
                }
                if (lay === 'timeline-horizontal') return { position: 'relative', textAlign: s.alignment || 'left' };
                if (lay === 'classic-side') {
                    return {
                        display: 'flex', gap: cbNum(s.iconSpacing, 16) + 'px', alignItems: 'flex-start',
                        flexDirection: s.contentAlignment === 'right' ? 'row-reverse' : 'row',
                    };
                }
                if (lay === 'clean-horizontal') {
                    return { display: 'flex', gap: Math.max(cbNum(s.iconSpacing, 16), 20) + 'px', alignItems: 'flex-start' };
                }
                if (lay === 'classic-title') return { textAlign: s.alignment || 'left' };
                return { display: 'flex', flexDirection: 'column', alignItems: contentBoxFlexAlign(el), textAlign: s.alignment || 'left' };
            };
            // Rules an inline style can't carry: the timeline rails and the Read More hover.
            const contentBoxCss = (el) => {
                const s = el.settings || {}, k = '.fcb-c-' + el.id;
                let css = k + ' .fcb-content>*:first-child{margin-top:0}' + k + ' .fcb-content>*:last-child{margin-bottom:0}';
                const lineW = Math.max(1, cbNum(s.timelineLineWidth, 2));
                const lineS = s.timelineLineStyle || 'solid';
                const lineC = s.timelineLineColor || '#e5e7eb';
                const inset = contentBoxInset(el);
                if (s.boxLayout === 'timeline-vertical') {
                    // Reach across this item's bottom inset, the row gap and the next item's top
                    // inset, or box padding would break the connector into disconnected pieces.
                    const reach = cbNum(s.rowGap, 30) + 2 * inset;
                    css += k + ' .fcb-line{position:absolute;left:50%;transform:translateX(-50%);top:' + contentBoxDot(el) + 'px;bottom:-' + reach + 'px;border-left:' + lineW + 'px ' + lineS + ' ' + lineC + ';}';
                    css += k + ' .fcb-item:last-child .fcb-line{display:none;}';
                }
                if (s.boxLayout === 'timeline-horizontal') {
                    const out = inset + Math.round(cbNum(s.columnGap, 30) / 2);
                    css += k + ' .fcb-rail-h{margin-left:-' + out + 'px;margin-right:-' + out + 'px;}';
                    css += k + ' .fcb-seg{border-top:' + lineW + 'px ' + lineS + ' ' + lineC + ';}';
                    // Trim the rail at each ROW edge, not just the first/last item — the grid wraps
                    // when there are more items than columns.
                    const n = contentBoxCols(el);
                    css += k + ' .fcb-item:nth-child(' + n + 'n+1) .fcb-seg-l,'
                         + k + ' .fcb-item:nth-child(' + n + 'n) .fcb-seg-r,'
                         + k + ' .fcb-item:last-child .fcb-seg-r{visibility:hidden;}';
                }
                if (s.linkHoverColor) {
                    css += k + ' .fcb-more:hover{color:' + (s.linkType === 'button' ? (s.linkButtonTextColor || '#ffffff') : s.linkHoverColor) + ' !important;';
                    if (s.linkType === 'button') css += 'background:' + s.linkHoverColor + ' !important;';
                    css += '}';
                }
                return css;
            };
            ...
            const addElement = (type) => {
                ...
                const newEl = {
                    ...
                    settings: {
                        ...
                        ...(type === 'content_box' ? {
                            boxLayout: 'classic-top',
                            columns: 3,
                            alignment: 'left',
                            contentAlignment: 'left',
                            linkType: 'text', linkArea: 'link', linkTarget: '_self', linkArrow: true,
                            columnGap: 30, rowGap: 30,
                            items: [
                                { id: Date.now() + '_0', icon: 'fas fa-star', iconColor: '', image: '', bgColor: '',
                                  title: 'Content Box', content: '<p>Your content goes here. Edit or remove this text inline.</p>',
                                  linkText: '', linkUrl: '' },
                            ],
                            // Icon
                            iconSize: 32, iconColor: '#2271b1', iconBgColor: '',
                            iconBorderRadius: 50, iconPadding: 0, iconSpacing: 16,
                            // Title
                            titleTag: 'h3', titleFontFamily: 'inherit',
                            titleFontSize: 20, titleFontSizeUnit: 'px', titleFontWeight: '600',
                            titleColor: '#222222', titleLineHeight: 1.3, titleLetterSpacing: '0px',
                            titleTextTransform: 'none', titleSpacing: 10,
                            // Content
                            contentFontFamily: 'inherit', contentFontSize: 15, contentFontSizeUnit: 'px',
                            contentFontWeight: '400', contentColor: '#666666',
                            contentLineHeight: 1.6, contentLetterSpacing: '0px',
                            // Read more
                            linkFontFamily: 'inherit', linkFontSize: 14, linkFontSizeUnit: 'px',
                            linkFontWeight: '600', linkTextTransform: 'none', linkColor: '', linkHoverColor: '',
                            linkSpacing: 14, linkButtonTextColor: '#ffffff',
                            linkButtonPaddingX: 20, linkButtonPaddingY: 10, linkButtonRadius: 4,
                            // Box chrome
                            boxBgColor: '', boxPadding: '', boxBorderRadius: 0,
                            boxBorderWidth: 0, boxBorderColor: '#e5e7eb', boxShadow: 'none',
                            // Timeline rail
                            timelineLineColor: '#e5e7eb', timelineLineWidth: 2, timelineLineStyle: 'solid',
                            timelineDotSize: 16, timelineDotColor: '', timelineDotBorderWidth: 0,
                            timelineDotBorderColor: '#ffffff', timelineGap: 22, timelineIconColor: '#ffffff',
                            marginTop: 0, marginTopUnit: 'px',
                            marginBottom: 0, marginBottomUnit: 'px',
                            cssClass: '', cssId: '',
                            visibility: { mobile: true, tablet: true, desktop: true },
