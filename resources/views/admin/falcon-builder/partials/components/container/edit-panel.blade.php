                ...
                <div v-show="{{ $base }}.settings.bgType === 'image'" class="space-y-4 border border-slate-100 rounded-md p-2">
                    ...
                    <div class="border-b border-slate-100 pb-3">
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <div class="flex gap-1 items-center">
                                <button @click.stop="openDynSrcMenu({{ $base }}.settings, 'bgImageDynamicSource', 'image', $event)"
                                        class="w-6 h-6 flex items-center justify-center rounded border transition-all"
                                        :class="{{ $base }}.settings.bgImageDynamicSource ? 'bg-[#2271b1]/10 text-[#0091ea] border-[#0091ea]/30' : 'bg-slate-50 text-slate-400 border-slate-200 hover:bg-slate-100'"
                        ...
                        <div v-if="{{ $base }}.settings.bgImageDynamicSource"
                             ...
                             @click.stop="openDynSrcMenu({{ $base }}.settings, 'bgImageDynamicSource', 'image', $event)">
                            <div class="flex items-center gap-2">
                                <i :class="['fa', getDynSrcDef({{ $base }}.settings.bgImageDynamicSource).icon, 'text-[#0091ea] text-sm']"></i>
                                <span class="text-[12px] font-bold text-[#0091ea]" v-text="getDynSrcDef({{ $base }}.settings.bgImageDynamicSource).label"></span>
                            </div>
                            <button @click.stop="{{ $base }}.settings.bgImageDynamicSource = ''"
                        ...
                        <div v-else-if="getResponsiveVal({{ $base }}.settings, 'bgImage', device)" class="relative group">
                    ...
                    <template v-if="getResponsiveVal({{ $base }}.settings, 'bgImage', device) || {{ $base }}.settings.bgImageDynamicSource">
                        <!-- Skip Lazy Loading -->
                        <div class="border-b border-slate-100 pb-3">
                            <div class="flex justify-between items-center mb-2">
                ...
                <template v-if="{{ $base }}.settings.sticky">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <button @click="{{ $base }}.settings.stickyBgColor = ''" class="text-slate-300 hover:text-red-400 transition-colors" title="Reset">
                                <i class="fa fa-undo text-[10px]"></i>
                            </button>
                        </div>
                        <div class="flex gap-2 items-center">
                            <div class="checkerboard rounded overflow-hidden w-8 h-8 flex-shrink-0 border border-slate-200">
                                <div @click="openColorPicker($event, {{ $base }}.settings, 'stickyBgColor', 'stickyBgColorOpacity')"
            ...
            <div class="border-t border-slate-100 pt-4">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <select v-model="{{ $base }}.settings.anim_type"
                                class="w-full border border-slate-200 rounded px-2 py-1.5 text-[11px] text-[#444] focus:outline-none focus:border-[#0091ea]">
                    ...
                    <template v-if="{{ $base }}.settings.anim_type">
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <label class="text-[10px] text-slate-500 block mb-1.5">Duration (ms)</label>
                                <input type="number" v-model.number="{{ $base }}.settings.anim_duration" placeholder="600" min="100" max="3000" step="100"
                                       class="w-full border border-slate-200 rounded px-2 py-1.5 text-[11px] text-[#444] focus:outline-none focus:border-[#0091ea]">
                            </div>
                            <div>
                                <label class="text-[10px] text-slate-500 block mb-1.5">Delay (ms)</label>
                                <input type="number" v-model.number="{{ $base }}.settings.anim_delay" placeholder="0" min="0" max="3000" step="100"
                                       class="w-full border border-slate-200 rounded px-2 py-1.5 text-[11px] text-[#444] focus:outline-none focus:border-[#0091ea]">
                            </div>
                        </div>
                        <div>
                            <label class="text-[10px] text-slate-500 block mb-1.5">Easing</label>
                            <select v-model="{{ $base }}.settings.anim_easing"
                                    class="w-full border border-slate-200 rounded px-2 py-1.5 text-[11px] text-[#444] focus:outline-none focus:border-[#0091ea]">
            ...
            <div class="border-t border-slate-100 pt-4">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <select :value="{{ $base }}.settings.vis_condition || ''"
                                @change="{{ $base }}.settings.vis_condition = $event.target.value"
                                class="w-full border border-slate-200 rounded px-2 py-1.5 text-[11px] text-[#444] focus:outline-none focus:border-[#0091ea]">
                    ...
                    <template v-if="({{ $base }}.settings.vis_condition || '') === 'schedule'">
                        <div>
                            <label class="text-[10px] text-slate-500 block mb-1.5">Show From</label>
                            <input type="datetime-local" v-model="{{ $base }}.settings.vis_date_from"
                                   class="w-full border border-slate-200 rounded px-2 py-1.5 text-[11px] text-[#444] focus:outline-none focus:border-[#0091ea]">
                        </div>
                        <div>
                            <label class="text-[10px] text-slate-500 block mb-1.5">Show Until</label>
                            <input type="datetime-local" v-model="{{ $base }}.settings.vis_date_to"
                                   class="w-full border border-slate-200 rounded px-2 py-1.5 text-[11px] text-[#444] focus:outline-none focus:border-[#0091ea]">
                        </div>
                    </template>
                </div>
