<x-falcon-cms::layouts.admin title="Edit Page">
    ...
    <div id="autosave-recovery" class="flex items-center gap-3 bg-[#fcf9e8] border border-[#f0e6a6] text-[#8a6d3b] rounded p-3 mb-4 text-[13px]">
        ...
        <form action="{{ route('admin.pages.revisions.restore', ['id' => $page->id, 'revision' => $pendingAutosave['id']]) }}" method="POST" class="ml-auto">
            @csrf
    ...
    <form action="{{ route('admin.pages.update', $page) }}" method="POST" id="page-form" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <input type="hidden" name="status" id="status-hidden" value="{{ $page->status }}">
        ...
        <div class="flex flex-col lg:flex-row gap-5">
            ...
            <div class="flex-grow min-w-0">
                ...
                @php
                    $isBuilderActive = request('start_builder') || old('editor_type', $page->editor_type) === 'builder';
                @endphp
                ...
                <div id="rich-editor-container" class="bg-white border border-[#dcdcde] rounded-sm p-0 {{ $isBuilderActive ? 'hidden' : '' }}">
                    <textarea id="wp-editor" name="content" rows="20">{{ old('content', $page->content) }}</textarea>
                </div>
            ...
            <div class="w-full lg:w-[280px] shrink-0 space-y-5">
                ...
                <div class="wp-metabox mb-0">
                    ...
                    <div class="wp-metabox-content" style="padding: 10px;">
                        ...
                        <div class="text-[13px] text-[#646970] space-y-3 mb-4">
                            ...
                            <div class="flex items-start">
                                ...
                                <div class="flex-grow">
                                    Status: <strong class="text-black" id="status-display-text">{{ ucfirst($page->status) }}</strong> <a href="#" class="text-[#2271b1] underline ml-1 toggle-publish-edit" data-target="status-edit">Edit</a>
                            ...
                            <div class="flex items-start">
                                ...
                                <div class="flex-grow">
                                    <span id="publish-display-prefix">{{ $page->status == 'scheduled' ? 'Scheduled for' : 'Published on' }}</span> <strong class="text-black" id="publish-display-text">{{ $page->published_at ? $page->published_at->timezone(cms_timezone())->format('M j, Y @ H:i') : $page->created_at?->timezone(cms_timezone())->format('M j, Y @ H:i') }}</strong> <a href="#" class="text-[#2271b1] underline ml-1 toggle-publish-edit" data-target="publish-edit">Edit</a>
                                    <div id="publish-edit" class="hidden mt-2 p-3 bg-[#f6f7f7] border border-[#dfdfdf] rounded">
                                        @php
                                            $months = ['01'=>'Jan','02'=>'Feb','03'=>'Mar','04'=>'Apr','05'=>'May','06'=>'Jun','07'=>'Jul','08'=>'Aug','09'=>'Sep','10'=>'Oct','11'=>'Nov','12'=>'Dec'];
                                            $pDate = ($page->published_at ?: $page->created_at ?: now())->copy()->timezone(cms_timezone());
                                            $curY  = (int) date('Y');
                    ...
                    <div class="border-t border-[#dfdfdf] px-3 py-2 flex items-center justify-between">
                        ...
                        <a href="{{ route('admin.pages.revisions', $page->id) }}" class="text-[#2271b1] hover:underline text-[13px] font-semibold">Browse</a>
                    </div>
                ...
                <div class="wp-metabox mb-0">
                    ...
                    <div class="wp-metabox-content">
                        <div id="fi-preview-container" class="{{ $page->featured_image ? '' : 'hidden' }} mb-3">
                            <img id="fi-preview" src="{{ $page->featured_image ? asset('storage/'.$page->featured_image) : '' }}" class="max-w-full h-auto border border-gray-200 p-1 bg-white cursor-pointer">
                        </div>
                        <a href="#" id="set-fi-btn" class="text-[#2271b1] text-[13px] underline {{ $page->featured_image ? 'hidden' : '' }}">Set featured image</a>
                        <a href="#" id="remove-fi-btn" class="text-[#b32d2e] text-[13px] underline {{ $page->featured_image ? '' : 'hidden' }} mt-2">Remove featured image</a>
                        <input type="hidden" name="featured_image" id="fi-path-hidden" value="{{ $page->featured_image }}">
    ...
    <form action="{{ route('admin.pages.destroy', $page) }}" method="POST" id="trash-form-main" class="hidden">
        @csrf @method('DELETE')
    </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('cancel-slug-btn')?.addEventListener('click', (e) => { 
                e.preventDefault(); if (slugInput) slugInput.value = originalSlug; viewSpan?.classList.remove('hidden'); editSpan?.classList.add('hidden'); 
            });
            ...
            if (editorTypeHidden) switchEditorMode(editorTypeHidden.value);
    ...
    {{-- ── Page: Revisions + Autosave ── --}}
    <script>
    (function () {
        const autosaveUrl = "{{ route('admin.pages.autosave', $page->id) }}";
        const csrf = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '{{ csrf_token() }}';
        const titleEl  = document.getElementById('title-input');
        const statusEl = document.getElementById('autosave-status');

        const getEditor  = () => (window.tinymce && tinymce.get('wp-editor')) || null;
        const getContent = () => { const ed = getEditor(); if (ed) return ed.getContent(); const ta = document.getElementById('wp-editor'); return ta ? ta.value : ''; };
        ...
        const isChanged = () => { const ed = getEditor(); return titleDirty || (ed ? ed.isDirty() : false); };
        ...
        async function autosave() {
            ...
            try {
                ...
                if (d.success) {
                    baselineTitle = getTitle(); titleDirty = false;
                    const ed = getEditor(); if (ed) ed.setDirty(false);
