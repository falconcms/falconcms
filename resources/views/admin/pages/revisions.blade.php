<x-falcon-cms::layouts.admin>
    <x-slot name="title">Revisions — {{ $page->title }}</x-slot>
    ...
    <div class="px-2">
        <div class="flex items-center justify-between mb-5 flex-wrap gap-3">
            <div>
                ...
                <p class="text-[13px] text-[#646970] mt-0.5">Compare versions of <strong>{{ $page->title }}</strong> and restore any of them.</p>
            </div>
            <div class="flex items-center gap-2">
                @if(count($entries) > 1)
                <form action="{{ route('admin.pages.revisions.clear', $page->id) }}" method="POST"
                      onsubmit="return confirm('Delete ALL revisions for this page? This cannot be undone (live content stays intact).')">
                    @csrf @method('DELETE')
                ...
                @endif
                <a href="{{ route('admin.pages.edit', $page) }}" class="wp-btn-secondary">&larr; Back to editor</a>
            </div>
        </div>
        ...
        @if(count($entries) <= 1)
            <div class="bg-white border border-[#dfdfdf] rounded p-10 text-center text-[#646970] text-[14px]">
                No revisions yet. Edit and update this page to start building its revision history.
            </div>
        @else
        <form method="GET" action="{{ route('admin.pages.revisions', $page->id) }}" id="compare-form"
            ...
            <div>
                ...
                <select name="from" onchange="document.getElementById('compare-form').submit()" class="wp-input text-[13px] min-w-[260px]">
                    @foreach($entries as $key => $e)<option value="{{ $key }}" {{ (string)$from === (string)$key ? 'selected' : '' }}>{{ $e['label'] }}</option>@endforeach
                </select>
            </div>
            <div class="text-[#646970] pb-2">&rarr;</div>
            <div>
                <label class="block text-[11px] font-bold text-[#646970] uppercase mb-1">To (newer)</label>
                <select name="to" onchange="document.getElementById('compare-form').submit()" class="wp-input text-[13px] min-w-[260px]">
                    @foreach($entries as $key => $e)<option value="{{ $key }}" {{ (string)$to === (string)$key ? 'selected' : '' }}>{{ $e['label'] }}</option>@endforeach
                </select>
            </div>
        </form>
        ...
        <div class="flex flex-col lg:flex-row gap-5">
            <div class="flex-grow min-w-0">
                ...
                <div class="bg-white border border-[#dfdfdf] rounded overflow-hidden">
                    ...
                    <div class="max-h-[60vh] overflow-y-auto py-2">{!! $diff !!}</div>
                </div>
            </div>
                            ...
                            <div class="flex items-center justify-between gap-2">
                                <div class="min-w-0">
                                    <div class="text-[12px] font-semibold text-[#1d2327] flex items-center gap-1.5">
                                        @if(($e['type'] ?? '') === 'autosave')<span class="text-[8px] font-bold uppercase bg-[#fcf3d6] text-[#8a6d3b] px-1 py-0.5 rounded">Autosave</span>
                                        @elseif(($e['type'] ?? '') === 'current')<span class="text-[8px] font-bold uppercase bg-[#e6ffed] text-[#11622b] px-1 py-0.5 rounded">Live</span>@endif
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    <form action="{{ route('admin.pages.revisions.restore', ['id' => $page->id, 'revision' => $key]) }}" method="POST"
                                    ...
                                    <form action="{{ route('admin.pages.revisions.delete', ['id' => $page->id, 'revision' => $key]) }}" method="POST"
