...
<head>
    ...
    <title>{{ $megaMenu['name'] }} | Mega Menu Builder</title>
...
<body class="bg-[#f1f1f1]">
    ...
    <script>
    function mmPoWidthTypeChange(type) {
        var row = document.getElementById('mm-custom-width-row');
        if (row) row.style.display = type === 'custom' ? 'block' : 'none';
    }
    async function mmPoSave() {
        var widthType   = document.getElementById('mm-width-type').value;
        var customWidth = parseInt(document.getElementById('mm-custom-width-input').value) || 1200;
        ...
        try {
            ...
            if (data.success) {
                var msg = document.getElementById('mm-po-saved-msg');
                if (msg) { msg.style.display = 'block'; setTimeout(function(){ msg.style.display = 'none'; }, 2500); }
            }
        } catch(e) { console.error('mmPoSave failed', e); }
