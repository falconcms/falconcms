<x-falcon-cms::layouts.admin>
    ...
    <div class="px-2">
        ...
        @if(session('success'))
            <div class="bg-[#ecf7ed] border-l-4 border-[#46b450] p-3 mb-5 max-w-[800px] rounded-sm text-[13px] text-[#1d2327]">
                {{ session('success') }}
            </div>
        @endif

        @if($errors->any())
            <div class="bg-[#fcf0f1] border-l-4 border-[#d63638] p-3 mb-5 max-w-[800px] rounded-sm">
                <p class="text-[13px] font-semibold text-[#1d2327] mb-1">Please fix the following:</p>
                <ul class="list-disc ml-5 text-[12px] text-[#d63638]">
        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="max-w-[800px]">
            ...
            <table class="w-full border-separate border-spacing-y-6">
                <tr>
                    ...
                    <td>
                        <input type="text" name="username" id="username" value="{{ old('username', $user->username) }}" class="wp-input w-[400px] h-8 shadow-sm" required>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" class="wp-input w-[400px] h-8 shadow-sm" required>
                ...
                <tr>
                    ...
                    <td>
                        <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" class="wp-input w-[400px] h-8 shadow-sm" required>
