<x-falcon-cms::layouts.admin title="Edit Post Type">
    ...
    <div class="max-w-[1280px] mx-auto pb-12" x-data="cptForm()">
        <form action="{{ route('admin.acpt.cpt.update', $postType->id) }}" method="POST">
            @csrf
            @method('PUT')

            <!-- Header -->
            <div class="flex items-center justify-between mb-4 mt-2">
                <h1 class="text-[22px] font-normal text-[#1d2327]">Edit Post Type</h1>
            ...
            <div class="bg-white border border-[#c3c4c7] shadow-[0_1px_1px_rgba(0,0,0,0.04)] rounded-[4px] mb-6 p-6 space-y-6">
                ...
                <div class="grid grid-cols-[200px_1fr] items-start">
                    ...
                    <div class="w-full max-w-[400px]">
                        <input type="text" name="plural_label" x-model="pluralLabel" required class="w-full border-[#8c8f94] focus:border-[#2271b1] border py-1.5 px-3 rounded-[3px] shadow-[inset_0_1px_2px_rgba(0,0,0,0.07)] text-[14px]">
                    </div>
                </div>
                ...
                <div x-show="tab === 'fields'" class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-[200px_1fr] items-start">
                        ...
                        <div>
                            <div class="grid grid-cols-3 gap-y-3 gap-x-6 text-[13px] text-[#2c3338] max-w-[600px]">
                                <label class="flex items-center"><input type="checkbox" name="supports[]" value="title" class="mr-2 border-[#8c8f94] rounded-[2px]" {{ in_array('title', $postType->supports ?? []) ? 'checked' : '' }}> Title</label>
                                <label class="flex items-center"><input type="checkbox" name="supports[]" value="editor" class="mr-2 border-[#8c8f94] rounded-[2px]" {{ in_array('editor', $postType->supports ?? []) ? 'checked' : '' }}> Editor</label>
                                <label class="flex items-center"><input type="checkbox" name="supports[]" value="excerpt" class="mr-2 border-[#8c8f94] rounded-[2px]" {{ in_array('excerpt', $postType->supports ?? []) ? 'checked' : '' }}> Excerpt</label>
                                <label class="flex items-center"><input type="checkbox" name="supports[]" value="featured_image" class="mr-2 border-[#8c8f94] rounded-[2px]" {{ in_array('featured_image', $postType->supports ?? []) ? 'checked' : '' }}> Featured Image</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-[200px_1fr] items-start">
                        ...
                        <div>
                            <label class="flex items-center text-[13px] text-[#2c3338]">
                                <input type="checkbox" name="show_in_menu" value="1" class="mr-2 border-[#8c8f94] rounded-[2px]" {{ $postType->show_in_menu ? 'checked' : '' }}>
                                Show on menu?
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-[200px_1fr] items-start">
                        ...
                        <div>
                            ...
                            <label class="flex items-center text-[13px] text-[#2c3338]">
                                <input type="checkbox" name="is_public" value="0" class="mr-2 border-[#8c8f94] rounded-[2px]" {{ !$postType->is_public ? 'checked' : '' }}>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('cptForm', () => ({
                pluralLabel: '{{ addslashes($postType->name) }}',
                singularLabel: '{{ addslashes($postType->singular_name) }}',
                postTypeKey: '{{ addslashes($postType->slug) }}',
                advanced: {{ $advancedOpen ? 'true' : 'false' }},
                tab: 'fields',
                selectedIcon: @json($postType->icon ?: '<svg class="w-full h-full" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>'),
                ...
                availableIcons: {
                    ...
                    'Microphone': '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 11-6 0v6a3 3 0 01-3 3z"/></svg>',
